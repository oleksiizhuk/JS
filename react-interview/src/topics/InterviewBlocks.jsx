...
export function CodeBlock({ ru, en }) {
  const lang = useLang();
...
function pick(lang, ru, en, children) {
  if (lang === "en" && en) return en;
  return ru ?? children;
}

export function InterviewQuestion({ ru, en, children }) {
  const lang = useLang();
  return (
...
export function ModelAnswer({ ru, en, children }) {
  const lang = useLang();
  return (
...
// items: [{ title, code?, text, en?: { title, code?, text } }]
// en у пункта не задан — показываем ru-версию (безопасная миграция).
export function Gotchas({ items }) {
  const lang = useLang();
  return (
    <div className="card">
