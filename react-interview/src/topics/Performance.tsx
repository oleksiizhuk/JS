...
import { InterviewQuestion, ModelAnswer, SectionTitle, CodeBlock, L } from "./InterviewBlocks";
import { useLang } from "../LangContext";

const Row = memo(function Row({ item, onSelect }: { item: string; onSelect: (item: string) => void }) {
...
export default function Performance() {
  ...
  const unstableHandler = (item: string) => setSelected(item);

  // ✅ useCallback — та же ссылка между рендерами → memo работает
  const stableHandler = useCallback((item: string) => setSelected(item), []);
