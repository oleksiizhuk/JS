...
// без ?. было бы TypeError: Cannot read properties of undefined`}
        // function вместо стрелки, чтобы описать TS внешний this (для стрелки он лексический);
        // Demo вызывает run(log) без this → this === undefined, как в модуле
        run={function (this: { name?: string } | undefined, log) {
          const obj = {
            name: "obj",
            regular() {
              return this.name;
