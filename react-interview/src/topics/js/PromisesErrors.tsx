...
Promise.any([fail("e1", 5), ok("первый успешный", 20)])
  .then(r => console.log("any:", r));`}
        run={(log) => {
          const ok = (v: string, ms: number) =>
            new Promise<string>((r) => setTimeout(() => r(v), ms));
          const fail = (e: string, ms: number) =>
            new Promise<never>((_, rj) => setTimeout(() => rj(new Error(e)), ms));
          Promise.all([ok("A", 10), fail("бум", 5)])
            .then((r) => log("all:", r))
...
(async () => {
  ...
  console.log("параллельно ~", Date.now() - t2, "мс");
        ...
        run={(log) => {
          const load = (name: string, ms: number) =>
            new Promise<string>((r) => setTimeout(() => r(name), ms));
          (async () => {
            const t1 = Date.now();
