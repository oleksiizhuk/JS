...
let x = 5;`}
        run={(log) => {
          // TS запрещает прямое обращение до объявления — читаем через замыкание,
          // runtime тот же: x в TDZ → ReferenceError
          const read = () => x;
          log(read()); // TDZ
          let x = 5;
        }}
...
let y = 1;`}
        run={(log) => {
          // необъявленный идентификатор = отсутствующее свойство globalThis
          log(typeof (globalThis as Record<string, unknown>).notDeclared);
          // typeof y до let — через замыкание (TS не пропускает напрямую), runtime тот же
          const probe = () => typeof y;
          log(probe());
          let y = 1;
        }}
...
f(); // 💥 ReferenceError: b в TDZ, когда вычисляется a = b`}
        run={(log) => {
          // TS запрещает a = b напрямую — читаем b через колбэк, runtime тот же: b в TDZ
          const read = (get: () => number) => get();
          function f(a = read(() => b), b = 2) {
            return [a, b];
          }
          log(f());
        }}
