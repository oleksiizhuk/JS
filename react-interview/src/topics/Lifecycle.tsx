...
import { InterviewQuestion, ModelAnswer, CodeBlock, L } from "./InterviewBlocks";
...
// ─────────────────────────────────────────────────────────────
function createLogStore(prefix: string) {
  let lines: string[] = [];
...
function LogPanel({ store }: { store: LogStore }) {
  const lang = useLang();
...
type ClassChildProps = { lang: Lang };
type ClassChildState = { n: number };

class ClassChild extends Component<ClassChildProps, ClassChildState, string> {
  constructor(props: ClassChildProps) {
    super(props);
    this.state = { n: 0 };
    clsStore.log(
  ...
  static getDerivedStateFromProps(props: ClassChildProps, state: ClassChildState) {
    clsStore.log(
      t(
        props.lang,
  ...
  shouldComponentUpdate(nextProps: ClassChildProps, nextState: ClassChildState) {
    clsStore.log(
      t(
        this.props.lang,
  ...
  getSnapshotBeforeUpdate(prevProps: ClassChildProps, prevState: ClassChildState) {
    clsStore.log(
      t(
        this.props.lang,
  ...
  componentDidUpdate(prevProps: ClassChildProps, prevState: ClassChildState, snapshot?: string) {
    clsStore.log(
      t(
        this.props.lang,
...
type ErrorBoundaryProps = { lang: Lang; children?: ReactNode };
