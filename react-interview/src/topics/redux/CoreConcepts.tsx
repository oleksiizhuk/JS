...
function createStore<S, A extends { type: string }>(
  reducer: (state: S | undefined, action: A | InitAction) => S
) {
...
type TodosState = { todos: string[]; filter: string };
type TodosAction =
  | { type: "todos/added"; payload: string }
  | { type: "todos/cleared" }
  | { type: "filter/changed"; payload: string };

function todosReducer(
  state: TodosState = { todos: [], filter: "all" },
  action: TodosAction | InitAction
): TodosState {
  switch (action.type) {
    case "todos/added":
...
const selectCount = (s: TodosState) => s.todos.length;
const selectLast = (s: TodosState) => s.todos[s.todos.length - 1] ?? "—";
