...
const fakeApi = () =>
  new Promise<User>((resolve) => setTimeout(() => resolve({ name: "Olex", id: 7 }), 900));

type UserAction =
  | { type: "user/pending" }
  | { type: "user/fulfilled"; payload: User }
  | { type: "user/rejected"; error: string };
type Thunk = (dispatch: Dispatch) => Promise<void>;
type Dispatch = (action: UserAction | Thunk) => void | Promise<void>;
type FetchState = { status: "idle" | "loading" | "done"; user: User | null };

export default function SideEffects() {
  const lang = useLang();
  const [state, setState] = useState<FetchState>({ status: "idle", user: null });
