...
import type { Question, Options } from "./types";
...
function localized(q: Question, lang: Lang): Question {
...
function QuestionText({ text, bold = true }: { text: string; bold?: boolean }) {
...
const sectionOf = (q: Question) => q.t.split(" · ")[0];
...
function shuffleOptions(q: Question): Question {
  const order = shuffle(q.o.map((_, i) => i)); // перестановка 4 индексов — as Options безопасен
  return {
    ...q,
    o: order.map((i) => q.o[i]) as Options,
    a: order.indexOf(q.a),
    en: q.en?.o ? { ...q.en, o: order.map((i) => q.en.o[i]) as Options } : q.en,
  };
}
...
function pickQuestions(n: number, sections: string[]): Question[] {
...
type Answer = { q: Question; ok: boolean };
