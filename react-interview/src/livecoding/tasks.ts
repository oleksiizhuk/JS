...
// Функция пользователя — любой сигнатуры: тесты сами знают, как её звать.
// Может быть и классом (EventEmitter) — поэтому есть construct-сигнатура.
/* eslint-disable @typescript-eslint/no-explicit-any */
export type UserFn = { (...args: any[]): any; new (...args: any[]): any };
/* eslint-enable @typescript-eslint/no-explicit-any */
export type TaskTest = { name: string; run: (fn: UserFn) => void | Promise<void> };
export type Task = {
  id: string;
  title: string;
  level: Level;
  ...
  solution: string;
  notes: string;
  en: { title: string; brief: string; description: string; starter: string; solution: string; notes: string };
};
...
function flatten2(arr) {
    ...
    tests: [
      {
        ...
        run: (fn) =>
          assertEq(fn(["apple", "avocado", "banana"], (w: string) => w[0]), {
            a: ["apple", "avocado"],
            b: ["banana"],
          }),
      },
      {
        ...
        run: (fn) =>
          assertEq(fn([1, 2, 3, 4], (n: number) => (n % 2 ? "odd" : "even")), {
            odd: [1, 3],
            even: [2, 4],
          }),
      },
      { name: "пустой массив → {}", run: (fn) => assertEq(fn([], (x: unknown) => x), {}) },
    ],
  ...
  // твой код (подсказка: состояние — в замыкании)
    ...
    tests: [
      ...
      },
      {
        ...
        run: (fn) => {
          const g = fn((a: number, b: number) => a + b);
          assertEq(g(2, 3), 5);
    ...
    tests: [
      {
        ...
        run: (fn) => {
          let calls = 0;
          const m = fn((a: number, b: number) => { calls++; return a + b; });
          assertEq(m(1, 2), 3);
          assertEq(m(1, 2), 3);
          assertEq(calls, 1, "число вызовов");
        },
      },
      {
        ...
        run: (fn) => {
          let calls = 0;
          const m = fn((a: number) => { calls++; return a * 2; });
          m(1); m(2); m(1);
      ...
      },
      {
        ...
        run: (fn) => {
          let calls = 0;
          const m = fn((...a: unknown[]) => { calls++; return a.length; });
          assertEq(m(1, 2), 2);
          assertEq(m(12), 1);
          assertEq(calls, 2, "число вызовов");
        },
  ...
  // твой код (подсказка: fn.length — арность)
    ...
    tests: [
      {
        name: "add(1)(2)(3) → 6",
        run: (fn) => assertEq(fn((a: number, b: number, c: number) => a + b + c)(1)(2)(3), 6),
      },
      {
        name: "смешанно: add(1,2)(3) и add(1)(2,3)",
        run: (fn) => {
          const c = fn((a: number, b: number, c: number) => a + b + c);
      ...
      },
      {
        ...
        run: (fn) => assertEq(fn((a: number, b: number) => a * b)(3, 4), 12),
      },
      {
        name: "каррированные версии независимы",
        run: (fn) => {
          const c = fn((a: number, b: number) => a + b);
          const add5 = c(5);
          assertEq(add5(1), 6);
    ...
    tests: [
      {
        ...
        run: (my) => {
          function who(this: { name: string }, greet: string) { return greet + " " + this.name; }
      ...
      },
      {
        ...
        run: (my) => {
          function f(this: unknown) { return typeof this; }
      ...
      },
      {
        ...
        run: (my) => {
          const obj = { name: "B" };
          function who(this: { name: string }) { return this.name; }
          my.apply(who, [obj]);
      ...
      },
      {
        ...
        run: (my) => {
          function sum(this: { base?: number }, a: number, b: number) { return a + b + (this.base || 0); }
    ...
    tests: [
      ...
      },
      {
        ...
        run: async (fn) => {
          ...
          let threw = false;
          try { await fn(dead, 2); } catch (e) { threw = true; assertEq((e as Error).message, "always", "текст ошибки"); }
    ...
    tests: [
      {
        ...
        run: async (fn) => {
          let calls = 0, last: number | null = null;
          const d = fn((x: number) => { calls++; last = x; }, 30);
          d(1); d(2); d(3);
          await sleep(60);
          assertEq(calls, 1, "число вызовов");
  ...
  // твой код
    ...
    tests: [
      ...
      },
      {
        ...
        run: async (fn) => {
          const got: number[] = [];
          const d = fn((x: number) => got.push(x), 30, { leading: true });
          d(1); d(2); d(3);
      ...
      },
      {
        ...
        run: async (fn) => {
          const got: number[] = [];
          const d = fn((x: number) => got.push(x), 30, { leading: true, trailing: false });
          d(1); d(2);
          await sleep(60);
      ...
      },
      {
        ...
        run: async (fn) => {
          ...
          const obj = { value: 42, method: fn(function (this: { value: number }) { seen = this.value; }, 20) };
          obj.method();
          await sleep(40);
    ...
    tests: [
      {
        ...
        run: (EE) => {
          ...
          e.on("msg", (a: string, b: string) => (got = a + b));
          e.emit("msg", "hi ", "there");
