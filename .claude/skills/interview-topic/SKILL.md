...
"mini-interview" in a strict order. Reference sample: `src/topics/js/LetVarConst.tsx`.
...
3. **`<SectionTitle>Разбор с примерами</SectionTitle>`** + 2–4 demos:
   - JS topics — the `Demo` component from `js/JsDemo.tsx`: code on the page,
     a "▶ Run" button, real output, a `hint` with the explanation
   - React/Redux topics — interactive cards with render counters
     (`useRenderCount` from `helpers.tsx`), logs (`LogPanel`), badges
   ...
   - **code is ALWAYS visible next to the demo** (`<pre className="code">`):
     ...
     automatically (`src/highlight.ts`)
...
```jsx
import { Demo } from "./JsDemo.tsx"; // from the js/ folder; adjust the path elsewhere
import { InterviewQuestion, ModelAnswer, SectionTitle, Gotchas } from "../InterviewBlocks.tsx";
```
...
- Register a new topic in `src/App.tsx` in SECTIONS (section order:
  JS Core → React → Redux → ... → Тренировка → Разное).
- Add 2–3 quiz questions for the new topic to `src/quiz/questions.ts`
...
- After changes: `cd react-interview && npm run build && npm test` — both
  ...
  (`src/__tests__/translations.test.ts`): it fails if any page's
